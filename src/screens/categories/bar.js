...
class Bar extends Component {
  ...
  componentWillMount() {
    fetch(domain + '/read.php?category=bar')
      .then((response) => response.json())
      .then((responseJson) => {
        this.setState({ bar_items: [...this.state.bar_items, ...responseJson], isLoading: false });
      })
      .catch((error) => {
        console.error(error);
      });
  ...
  renderBarItems = () => {
    let bar_items = [];
    let stateItems = this.state.bar_items
    return (
      <FlatGrid
        items={stateItems}
