...
const _DATA = [{ id: 1, title: 'Saloons' }];
...
class Saloon extends Component {
  ...
  componentWillMount() {
    fetch(domain + '/read.php?category=saloon')
      .then((response) => response.json())
      .then((responseJson) => {
        this.setState({ saloon_items: [...this.state.saloon_items, ...responseJson], isLoading: false });
      })
      .catch((error) => {
        console.error(error);
      });
  ...
  renderSaloonItems = () => {
    let saloon_items = [];
    let stateItems = this.state.saloon_items
    return (
      <FlatGrid
        items={stateItems}
