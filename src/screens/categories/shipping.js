...
const _DATA = [{ id: 1, title: 'Shipping' }];
...
class Shipping extends Component {
  ...
  componentWillMount() {
    fetch(domain + '/read.php?category=shipping')
      .then((response) => response.json())
      .then((responseJson) => {
        this.setState({ shipping_items: [...this.state.shipping_items, ...responseJson], isLoading: false });
      })
      .catch((error) => {
        console.error(error);
      });
  ...
  renderShippingItems = () => {
    let shipping_items = [];
    let stateItems = this.state.shipping_items
    return (
      <FlatGrid
        items={stateItems}
