...
const _DATA = [{ id: 1, title: 'Car Wash' }];
...
class CarWash extends Component {
  ...
  componentWillMount() {
    fetch(domain + '/read.php?category=carwash')
      .then((response) => response.json())
      .then((responseJson) => {
        this.setState({ carwash_items: [...this.state.carwash_items, ...responseJson], isLoading: false });
      })
      .catch((error) => {
        console.error(error);
      });
  ...
  renderCarWashItems = () => {
    let carwash_items = [];
    let stateItems = this.state.carwash_items
    return (
      <FlatGrid
        items={stateItems}
