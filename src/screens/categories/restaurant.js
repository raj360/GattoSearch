...
const _DATA = [{ id: 1, title: 'Restaurants' }];
...
class Restaurant extends Component {
  ...
  componentWillMount() {
    fetch(domain + '/read.php?category=restaurant')
      .then((response) => response.json())
      .then((responseJson) => {
        this.setState({ restaurant_items: [...this.state.restaurant_items, ...responseJson], isLoading: false });
      })
      .catch((error) => {
        console.error(error);
      });
  ...
  renderRestaurantItems = () => {
    ...
    let stateItems = this.state.restaurant_items
    return (
      <FlatGrid
        items={stateItems}
