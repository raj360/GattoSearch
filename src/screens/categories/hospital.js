...
const _DATA = [{ id: 1, title: 'Hospitals' }];
...
class Hospital extends Component {
  ...
  componentWillMount() {
    fetch(domain + '/read.php?category=hospital')
      .then((response) => response.json())
      .then((responseJson) => {
        this.setState({ hospital_items: [...this.state.hospital_items, ...responseJson], isLoading: false });
      })
      .catch((error) => {
        console.error(error);
      });
  ...
  renderHospitalItems = () => {
    let hospital_items = [];
    let stateItems = this.state.hospital_items
    return (
      <FlatGrid
        items={stateItems}
