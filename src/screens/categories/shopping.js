...
const _DATA = [{ id: 1, title: 'Shopping' }];
...
class Shopping extends Component {
  ...
  componentWillMount() {
    fetch(domain + '/read.php?category=shopping')
      .then((response) => response.json())
      .then((responseJson) => {
        this.setState({ shopping_items: [...this.state.shopping_items, ...responseJson], isLoading: false });
      })
      .catch((error) => {
        console.error(error);
      });
  ...
  renderShoppingItems = () => {
    let shopping_items = [];
    let stateItems = this.state.shopping_items
    return (
      <FlatGrid
        items={stateItems}
